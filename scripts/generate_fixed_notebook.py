"""
Script para gerar um novo notebook com a correção do erro
"""
...
# Adicionar célula de markdown com título
nb['cells'] = [
    nbf.v4.new_markdown_cell('# Feature Engineering para Detecção de Fraudes\n\n'
                           'Este notebook implementa a criação de features para o modelo de detecção de fraudes em transações de cartão de crédito.')
]

# Adicionar célula de importações
imports_cell = """# 1. Setup e Importações
import pandas as pd
import numpy as np
...
# Adicionar célula para carregar dados
load_data_cell = """# 2. Carregar Dados e Insights da EDA
...
nb['cells'].append(nbf.v4.new_code_cell(load_data_cell))

# Adicionar função corrigida para contar transações nas últimas 24 horas
count_txns_function = """# Função corrigida para contar transações nas últimas 24 horas
...
nb['cells'].append(nbf.v4.new_code_cell(count_txns_function))

# Adicionar célula para features temporais
temporal_features_cell = """# 3. Feature Engineering
...
nb['cells'].append(nbf.v4.new_code_cell(temporal_features_cell))

# Adicionar célula para features de velocidade
velocity_features_cell = """# Features de Velocidade
...
nb['cells'].append(nbf.v4.new_code_cell(velocity_features_cell))

# Adicionar célula para features de valor
value_features_cell = """# 4. Features de Valor
...
nb['cells'].append(nbf.v4.new_code_cell(value_features_cell))

# Adicionar célula para features de localização
location_features_cell = """# 5. Features de Localização
...
nb['cells'].append(nbf.v4.new_code_cell(location_features_cell))

# Adicionar célula para salvar o dataset processado
save_data_cell = """# 6. Salvar Dataset Processado
...
nb['cells'].append(nbf.v4.new_code_cell(save_data_cell))

# Adicionar célula para mostrar resumo
summary_cell = """# Mostrar as primeiras linhas do dataset com as novas features
...
nb['cells'].append(nbf.v4.new_code_cell(summary_cell))

# Salvar o notebook
output_path = os.path.join(project_root, 'notebooks', '02_feature_engineering_fixed.ipynb')
with open(output_path, 'w') as f:
    nbf.write(nb, f)

print(f"Notebook corrigido salvo em: {output_path}")
